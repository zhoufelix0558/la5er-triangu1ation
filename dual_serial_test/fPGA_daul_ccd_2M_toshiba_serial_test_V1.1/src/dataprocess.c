...
float sum(float data[],int loop)
{
	float sum_float = 0;
	int cnt;
	for(cnt=0;cnt<loop;cnt++)
		sum_float += data[cnt];
	return fabs(sum_float);
}


float kalman_ratio_adjust(float data[],int loop)
{
	float sum_num , value;
	sum_num = sum(data,loop);
	value=38.46*sum_num-6.92;
	if (value <=-5)
		value = -5;
	if (value >0)
		value = 0;
...
float kalman_ratio_adjust_para(float data[],int loop,float k, float C_yaxis, float up_limit, float down_limit)
{
	float sum_num , value;
	sum_num = sum(data,loop);
	value=k*sum_num - C_yaxis;
	if (value <=-down_limit)
		value = -down_limit;
	if (value >up_limit)
		value = up_limit;
...
void str_get(char str[], char num[], int stpoint, int edpoint)
{
	int cnt = stpoint;
	for (; cnt <= edpoint; cnt++)
	{
		num[cnt - stpoint] = str[cnt];
	}
	num[cnt - stpoint] = 0;
}

/*
int receive_from_serial(char data[],int length)
{
	if
...
*/
void para_get(char buffer[], float* k, float* C_yaxis, float*up_limit, float*down_limit, float*deria_threshold, float*deria_speed)
{
	char num[7];
	str_get(buffer, num, 1, 6);
		*k = (float)atof(num);
	str_get(buffer, num, 7, 12);
		*C_yaxis = (float)atof(num);
	str_get(buffer, num, 13, 15);
		*up_limit = (float)atof(num);
	str_get(buffer, num, 16, 18);
		*down_limit = (float)atof(num);
	str_get(buffer, num, 19, 22);
		*deria_threshold = (float)atof(num);
	str_get(buffer, num, 23, 28);
		*deria_speed = (float)atof(num);
