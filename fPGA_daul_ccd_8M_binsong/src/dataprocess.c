...
float zhixin_upward(const float float_data[point_NUM], uint16_t startpoint, uint16_t endpoint,  float ratio ,uint16_t MaxValue)
{
	float thershold;
	float sum_jiaquan = 0.0;
	float sum_light = 0.0;
	...
	uint16_t cnt;
	//uint16_t step = (endpoint - startpoint) / thershod_step + 1;
	thershold = (float)(MaxValue)*(1-ratio) + ratio*(float)(float_data[20]+float_data[2510])/2.0;
	...
	for (cnt = startpoint; cnt <= endpoint; cnt ++) {
		...
		sum_jiaquan += ((float_data[cnt] - thershold) * (float_data[cnt] - thershold) * cnt / 10000);
		sum_light += (float_data[cnt] - thershold) * (float_data[cnt] - thershold) / 10000;
	}
...
uint16_t bi_side_search_float(const float data[point_NUM], uint16_t point_notclear, uint16_t direction, float ratio, uint16_t step,uint16_t MaxValue)
{
	int16_t onestep = 1;
	float threshold;
	uint16_t cnt;
	...
	//long i0;
	//	onestep = 1;
	threshold = (float)(MaxValue)*(1-ratio) + ratio*(float)(data[20]+data[2510])/2.0;
...
uint16_t bi_side_search_withMaxValue(const uint16_t data[point_NUM], uint16_t point_notclear, uint16_t direction, float ratio, int16_t step,uint16_t MaxValue)
{
	int16_t onestep = 1;
	uint16_t threshold;
	uint16_t cnt;
	//threshold = (uint16_t)(((float)(data[0] + data[2531]) / 2.0 - (float)data[point_notclear]) * ratio + (float)data[point_notclear]);
	threshold = (uint16_t)((float)(MaxValue)*(1-ratio) + ratio*(float)(data[20]+data[2510])/2.0);
	cnt = point_notclear;
	if (direction == 0) {		//左边
		step = -step;
	}
...
uint16_t point_nclear_return_maxValue(const uint16_t data[point_NUM], uint16_t  step, uint16_t *MaxValue)
{
	uint16_t num_max_tmp;
	uint16_t num_max_index_tmp;
	//uint16_t i0;
