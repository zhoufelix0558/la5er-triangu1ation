...
uint32_t UART_init(uart_regs_t *uart, uint32_t baud_rate)
{
  ...
  //divisor = (SYSCLOCK2_HZ / 2) / (baud_rate * 16);
  divisor = (456000000 / 2) / (baud_rate * 16);

  // divisor = (SYSCLOCK2_HZ / 2) / (baud_rate * 10);//uart1¡¢uart2
   uart->DLL = (uint8_t) divisor;
   uart->DLH = (uint8_t) (divisor >> 8);
   ...
   //uart->PWREMU_MGMT |= 0x6000;
#endif

   return (ERR_NO_ERROR);
}

void UART_init_rx(uart_regs_t *uart, uint32_t baud_rate)
{
   uint32_t divisor;
  ...
  // divisor = (SYSCLOCK2_HZ / 2) / (baud_rate * 10);//uart1¡¢uart2
   ...
   uart->FCR = 0;
   SETBIT(uart->FCR, FIFOEN);
   SETBIT(uart->FCR, RXFIFTL | RXCLR | TXCLR | DMAMODE1);
